...
from Book_demo.book_sort import get_detailed_info
...
def img_select(task_type):
    if task_type == 'label':
        ...
        st.markdown("""
            ...
            해당 매장의 이름과 지점을 인식하여 추천 메뉴를 제공합니다.
            </div>
            """, unsafe_allow_html=True)
        ...
        if input_option == "카메라":
            ...
            <div style="line-height: 1.5; font-size: 16px; border: 1px solid #ddd; padding: 10px; border-radius: 5px; background-color: #f9f9f9;">
                ...
                제목이 흐리거나 가려져 있으면 정확한 인식이 어려울 수 있습니다.
            </div>
            """, unsafe_allow_html=True)
            image_path = bring_img.upload_image()
        ...
        if st.button("크롭된 이미지 사용하기"):
            if task_type == 'label':
                ...
                # 로딩 메시지를 표시하면서 데이터 처리
                    with st.spinner("상품 리뷰수, 최저가, 평점을 분석 중입니다. 잠시만 기다려 주세요!"):
                ...
                st.json(result)
                with st.spinner("추천 메뉴를 분석 중입니다. 잠시만 기다려주세요!"):
                    best_menu = recommend_best_menus(result)
                st.subheader("🍽️메뉴 추천 결과🍽️")
                ...
                st.write("📖 AI가 도서에서 추출한 제목:")
                st.json(result)
                if result:
                # "product name" 키를 사용해 product_data 생성
                    title_data = [{"title": item["title"]} for item in result]

                    with st.spinner("도서 리뷰 수, 평점, 베스트셀러 순위를 분석 중입니다. 잠시만 기다려 주세요!"):
                    ...
                    # 리스트를 딕셔너리로 변환
                    book_data_dict= {item["title"]: item for item in book_data}
                    recommendation_df, recommendations = process_book_data(book_data_dict)
                    book_display_recommendations(recommendation_df, recommendations)

                    # 줄거리와 추천 독자 정보 가져오기
                    detailed_info_df = get_detailed_info(book_data)
